...
def create(request):
    ...
    if request.method == "POST":
        ...
        if start_time and end_time and text and xmname and level:
            ...
            if obj or cq_obj:
                print("写库")
                Info.objects.create(start_time=start_time,end_time=end_time,xmname=xmname,text=text,level=level,username=username,end=fault)
            else:
                message = "所选的项目不存在，请不要修改页面"
        else:
            status = False
            message = "所有项目不能为空"
        print(start_time,end_time,text,xmname)
        return HttpResponse(json.dumps({"data":{"message":message,"status":status}}))
    level_table = Info._meta.get_field("level").choices
    end_table = Info._meta.get_field("end").choices
    return render(request,'avail/create.html',{"all_group":all_group,"level_table":level_table,"end_table":end_table})
...
def avail_index(request):
    ...
    if timenow.year > int(this_year):
        ...
        now_week_2 = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[2]
        if now_week_2 < 7:
            last_day = last_day - (7 - now_week_2)
            now_week = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[1]
    else:
    ...
    week_day_2 = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[2]
    if week_day_2 < 7:
        last_day = last_day - (7 - week_day_2)
        week_day = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[1]
    ...
    return render(request,"avail/index.html",{"xm_list":xm_list,"left":xm_left,"right":xm_right,"week_list":week_list,
					"now_week":now_week,"last_week":week_day,"absolutely_list":absolutely_list,
...
def xm_chart(request):
    ...
    if timenow.year > int(this_year):
        ...
        now_week = timenow.isocalendar()[1]
    obj_list = Info.objects.filter(level=1,end=1,start_time__icontains=this_year,xmname=xmname).order_by("start_time")
    last_week_day = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[1]
    last_week_day_2 = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[2]
    if last_week_day_2 < 7:
        last_day = last_day - (7 - last_week_day_2)
        last_week_day = datetime.datetime(this_year, 12, last_day, 0, 0, 0).isocalendar()[1]
    for obj in obj_list:
...
def year_chart(request):
    ...
    cq_group = models.CQ_Group.objects.all()
    all_group = list(group) + list(cq_group)
    avail_group = Info.objects.filter(level=1,end=1,start_time__icontains=this_year).values('xmname').annotate(c=Count('xmname'))
    obj_list = Info.objects.filter(level=1,end=1, start_time__icontains=this_year).order_by("start_time")
